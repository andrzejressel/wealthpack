...
import {
    ...
    Services,
    stringToSupportedService,
    SupportedService,
    supportedServiceToDescription
} from "./types";
                ...
                <CardContent className="p-6">
                    <div className="w-48">
                        <div>
                                    ...
                                    <SelectContent className="max-h-[500px] overflow-y-auto">
                                        {data.map((account) => (
                                            <SelectItem value={account.id} key={account.id}>
                                                {account.name} <span
                                                className="text-muted-foreground font-light">({account.currency})</span>
                                            </SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                            ...
                            <Button onClick={handleSend} disabled={(accountId === null) || (service === null) || (file === null)}>Import</Button>
                        </div>
                    </div>
                </CardContent>
            </Card>
...
export default function enable(ctx: AddonContext) {
    ...
    // Add a route
    const Wrapper = () => <QueryClientProvider client={queryClient}><AddonExample ctx={ctx}/></QueryClientProvider>;
    ctx.router.add({
